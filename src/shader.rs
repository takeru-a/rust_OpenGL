...
impl Shader {
    ...
    pub fn with_geometry_shader(
        ...
        .unwrap_or_else(|_| panic!("failed to open file:{}", vertex_path));
        let mut fragment_file = File::open(fragment_path)
        .unwrap_or_else(|_| panic!("fail to open file:{}", fragment_path));
        ...
        unsafe {
            let vertex = gl::CreateShader(gl::VERTEX_SHADER);
            gl::ShaderSource(vertex, 1, &cstr_fragment_code.as_ptr(), ptr::null());
